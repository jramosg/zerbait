(ns user
  ...
  (:require
    ...
    [kit.zerbait.core :refer [start-app]]))
...
(defn dev-prep!
  ...
  (integrant.repl/set-prep! (fn []
                              (-> (kit.zerbait.config/system-config {:profile :dev})
                                  (ig/expand)))))

(defn test-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (kit.zerbait.config/system-config {:profile :test})
                                  (ig/expand)))))
...
(comment
  ...
  (def config {:store :database,
               ...
               :db {:connection-uri "jdbc:postgresql://localhost:5432/zerbait_dev?user=zerbait&password=zerbait"}}) 
